...
>
  ...
  >
    <ScrollArea.Root class="relative h-52">
      <ScrollArea.Viewport class="h-full w-full">
        <ScrollArea.Content>
          ...
          {#each options as item (item.value)}
            <Select.Item
              {...item}
              class="flex justify-between items-center gap-2 mt-2 leading-tight"
            >
      ...
      >
        <ScrollArea.Thumb
          class="relative flex-1 rounded-full bg-accent opacity-40 transition-opacity hover:opacity-100 cursor-pointer"
        />
      </ScrollArea.Scrollbar>
      <ScrollArea.Corner />
    </ScrollArea.Root>

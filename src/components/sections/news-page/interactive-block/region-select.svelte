<script lang="ts">
  ...
  const options: Selected<string>[] = regions.map(item => ({
    value: item.slug,
    label: item.name,
  }));

  $: selected = options.find(item => item.value === $query.region);
</script>
...
>
  ...
  >
    <Select.Value placeholder="Все регионы" />
    <svg
  ...
  >
    {#if $query?.region}
      <Select.Item value={null} label="Все регионы">
